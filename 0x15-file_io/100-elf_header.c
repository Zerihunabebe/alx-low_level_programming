 ...
 * @e_ident: pointer to an array containing the ELF magic number.
 *
 * Desc: Magic numbers are seprated by spaces
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: pointer to an array containing the ELF header
 */
void print_class(unsigned char *e_ident)
{
	printf(" Class:                                ");
...
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
	...
	if (e_ident[EI_DATA] == ELFDATA2MSBi)
	{
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	{
		close_elf(o);
		dprointf(STDERR_FILENO, "Error: can't read file %s\n", argv[1]);
		exit(98);
	}
